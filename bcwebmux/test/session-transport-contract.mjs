...
const { FrameType: F, MAX_FRAME_LENGTH, encodeFrame, decodeFrame, writeUint32LE, writeUint64LE, readUint32LE, readUint64LE } = await load("protocol.js");
...
class Clock {
  ...
  delay() { return this.next()?.[1].at - this.time; }
}
class Target {
  listeners = new Map();
  hidden = false;
...
class Socket extends Target {
  ...
  send(bytes) { this.sent.push(decodeFrame(bytes)); }
  // Deliberately never completes the close handshake.
  close() { this.closeCalls++; this.readyState = Socket.CLOSING; }
  closed() { this.readyState = Socket.CLOSED; this.emit("close"); }
  frame(type, payload = new Uint8Array(), fields = {}) {
    this.emit("message", { data: encodeFrame({ type, payload, connectionSequence: ++this.serverSequence, ...fields }) });
  }
  frames(type) { return this.sent.filter(frame => frame.type === type); }
  last(type) { return this.frames(type).at(-1); }
}
...
class Core {
  ...
  resizeCanonical() {}
  reset() {}
  write(bytes) { this.writes.push(bytes.slice()); }
  dispose() { this.disposed++; }
}
function harness(t, options = {}) {
  const clock = new Clock(), sockets = [], window = new Target(), document = new Target(), errors = [];
  const transport = new SessionTransport({
    url: "wss://contract.invalid/ws", clientInstanceId: "33333333-3333-4333-8333-333333333333",
    clock, random: () => 0.5, window, document,
...
function attachmentFields(attach) {
  return { attachmentId: attach.attachmentId, attachmentEpoch: attach.serverEpoch ?? 1n, sessionId: attach.sessionId };
}
async function begin(socket, mode = 2, controller = false, options = {}) {
  ...
  socket.frame(F.ATTACH_BEGIN, payload, { ...attachmentFields(attach), requestId: attach.requestId });
  await flush();
  return attach;
}
...
test("old callbacks and rejected async message work cannot retire or block replacement", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core();
  let rejectShadow;
  h.transport.activate({
    core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => new Promise((resolve, reject) => { rejectShadow = reject; }),
  });
  ...
  assert.equal(h.transport.state.connected, true, "new message chain is independent of old pending await");
...
test("input is never replayed and disconnected input is dropped", async t => {
  ...
  assert.equal(replacement.closeCalls, 0);
  assert.deepEqual(replacement.sent.map(frame => frame.type), [F.HELLO, F.ATTACH]);
...
test("stable attachment handover carries controller lease, ACK precedes input, and unchanged size does not resize", async t => {
  ...
  let resize;
  const terminal = {
    core, state: { physicalCellWidth: 8, physicalCellHeight: 16 },
    onResize: fn => { resize = fn; return { dispose() {} }; },
    onSelectionModeChange: () => ({ dispose() {} }),
    attachCore(value) { this.core = value; },
  };
...
test("matched PONG arrival bypasses deferred core creation while application frames stay ordered", async t => {
  ...
  const terminal = {
    core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => new Promise(resolve => { resolveShadow = resolve; }),
    attachCore(value) { this.core = value; },
  };
  h.transport.activate(terminal);
...
test("fast PONG path still enforces wire sequence, connection scope, and nonce", async t => {
  for (const kind of ["sequence", "scope", "nonce"]) {
    const h = harness(t), socket = await healthy(h);
    await h.clock.advance(1000);
    const ping = socket.last(F.PING);
...
test("genuine geometry changes made offline remain pending through carried-lease resume", async t => {
  ...
  assert.ok(replacement.sent.indexOf(replacement.frames(F.ACK).at(-1)) < replacement.sent.indexOf(request));
});

test("fatal ERROR during digest preserves committed core and retries after the floor", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core(), shadow = new Core();
  let snapshots = 0;
  shadow.restoreSnapshot = () => snapshots++;
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: async () => shadow, attachCore(value) { this.core = value; } });
  const attached = h.transport.attach(metadata, core); await flush();
  const initial = await begin(socket, 2, true); await live(socket, initial);
  const record = await attached;
  record.eventSeq = 7n; record.outputOffset = 9n;
  socket.emit("error"); await h.clock.advance(PROFILE.retryFloorMs);
  const replacement = await welcome(h); const attach = await begin(replacement, 1);
  ...
  try {
    ...
    record.checkpoint = new Uint8Array([1]); record.checkpointOffset = 1; record.checkpointHash = new Uint8Array(32);
    replacement.frame(F.CHECKPOINT_END, new Uint8Array(32), attachmentFields(attach));
    await flush(); assert.equal(typeof resolveDigest, "function");
    replacement.frame(F.ERROR, fatalPayload()); replacement.closed();
    assert.equal(h.errors.length, 1); assert.equal(h.transport.state.status, "backoff");
    assert.equal(record.core, core); assert.equal(record.eventSeq, 7n); assert.equal(record.outputOffset, 9n);
    resolveDigest(new ArrayBuffer(32)); await flush();
    assert.equal(snapshots, 0); assert.equal(shadow.disposed, 1); assert.equal(core.disposed, 0);
    assert.equal(replacement.frames(F.ACK).length, 0);
    assert.equal(h.transport.state.queuedApplicationBytes, 0);
    socket.closed();
    await h.clock.advance(PROFILE.retryFloorMs * 2 - 1); assert.equal(h.sockets.length, 2);
    await h.clock.advance(1); assert.equal(h.sockets.length, 3);
...
test("backoff has a positive floor and ceiling; WELCOME alone cannot reset it", async t => {
  ...
  for (const delay of [500, 1000, 2000, 4000, 8000, 10000, 10000, 10000]) {
    const socket = await welcome(h); await ready;
    socket.emit("error"); socket.closed();
    const count = h.sockets.length;
    h.window.emit("online");
    await h.clock.advance(delay - 1); assert.equal(h.sockets.length, count);
    await h.clock.advance(1); assert.equal(h.sockets.length, count + 1);
  }
  await welcome(h); await pong(h);
  h.sockets.at(-1).emit("error"); await h.clock.advance(PROFILE.retryFloorMs);
  assert.equal(h.sockets.length, 10, "a healthy connection resets retry backoff to its 500ms floor");
});
...
test("queued ACK stays ahead of input and controls get bounded reserved priority", async t => {
  ...
  await live(socket, attach); const record = await attached;
  assert.equal(record.live, true); assert.equal(socket.frames(F.ACK).length, 0);
  core.input(new Uint8Array([1])); assert.equal(socket.frames(F.INPUT).length, 0);
  socket.frame(F.PING, new Uint8Array(8)); assert.equal(socket.frames(F.PONG).length, 1);
  socket.bufferedAmount = 0; await h.clock.advance(100);
  assert.equal(socket.frames(F.ACK).length, 1);
  core.input(new Uint8Array([2])); assert.equal(socket.frames(F.INPUT).length, 1);
...
test("local core creation failure belongs to attachment operation, not connection failure", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core();
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: async () => { throw new Error("core allocation failed"); } });
  const attached = h.transport.attach(metadata, core);
  const rejected = assert.rejects(attached, /core allocation failed/);
  await flush(); await begin(socket, 1); await rejected;
  assert.equal(socket.closeCalls, 0); assert.equal(h.errors.length, 0);
  await h.clock.advance(PROFILE.probeIntervalMs); await pong(h);
  assert.equal(h.transport.state.connected, true);
});

test("attachment notifications are synchronous committed changes, separate from lifecycle", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core(), changes = [], statuses = [];
  ...
  const record = await attached;
  h.transport.setActive(record); assert.equal(changes.at(-1), "active");
  await lease(socket, attach); assert.equal(changes.at(-1), "lease");
  const inputAck = new Uint8Array(12); inputAck[8] = 2;
  socket.frame(F.INPUT_ACK, inputAck, attachmentFields(attach));
  assert.equal(changes.at(-1), "controller"); assert.equal(record.controller, false);
  await flush();
  socket.frame(F.EXITED, new Uint8Array(20), attachmentFields(attach));
  assert.equal(changes.at(-1), "exit");
  await flush();
  h.transport.detach(record); assert.equal(changes.at(-1), "removed");
...
test("attach cancellation before and while waiting for connect preserves caller reason", async t => {
  const h = harness(t), abort = new AbortController(), reason = new Error("caller canceled startup");
  abort.abort(reason);
  await assert.rejects(h.transport.attach(metadata, new Core(), { signal: abort.signal }), error => error === reason);
  ...
  next.abort(reason); await rejected;
...
test("abort during restore releases ordered work and late core without closing shared socket", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core(), abort = new AbortController();
  let resolveCore;
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => new Promise(resolve => { resolveCore = resolve; }) });
  const attached = h.transport.attach(metadata, core, { signal: abort.signal });
  const reason = new Error("caller canceled restore"), rejected = assert.rejects(attached, error => error === reason);
  await flush(); const attach = await begin(socket, 1);
  socket.frame(F.LIVE_BARRIER, new Uint8Array(16), attachmentFields(attach));
  abort.abort(reason); await rejected; await flush();
  assert.equal(h.transport.state.attachmentCount, 0);
  assert.equal(h.transport.state.queuedApplicationBytes, 0);
  assert.equal(socket.frames(F.DETACH).length, 1); assert.equal(socket.frames(F.ACK).length, 0);
  assert.equal(socket.closeCalls, 0);
  const late = new Core(); resolveCore(late); await flush(); assert.equal(late.disposed, 1);
  await h.clock.advance(PROFILE.probeIntervalMs); await pong(h); assert.equal(h.errors.length, 0);
});

test("abort after ATTACH submission detaches when its admitted server epoch arrives", async t => {
  const h = harness(t), socket = await healthy(h), abort = new AbortController();
  const attached = h.transport.attach(metadata, new Core(), { signal: abort.signal });
  const reason = new Error("cancel pending ATTACH"), rejected = assert.rejects(attached, error => error === reason);
  await flush(); abort.abort(reason); await rejected;
...
test("successful LIVE removes old abort signal; pending uncanceled attach survives retries", async t => {
  const h = harness(t), socket = await healthy(h), abort = new AbortController();
  const attached = h.transport.attach(metadata, new Core(), { signal: abort.signal }); await flush();
  socket.emit("error"); await h.clock.advance(PROFILE.retryFloorMs);
  const replacement = await welcome(h); const attach = await begin(replacement, 2, true);
  await live(replacement, attach); const record = await attached;
  abort.abort(new Error("old caller disposed"));
  assert.equal(record.live, true); assert.equal(record.active, true);
...
test("two actual core creations bound canceled retries until actual completion releases admission", async t => {
  const h = harness(t), core = new Core(), jobs = [];
  let socket = await healthy(h);
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => new Promise(resolve => jobs.push(resolve)) });
  const attached = h.transport.attach(metadata, core); attached.catch(() => {}); await flush();
  await begin(socket, 1); assert.equal(jobs.length, 1);
  for (let i = 0; i < 4; i++) {
    socket.emit("error"); socket.closed(); await h.clock.advance(i === 0 ? PROFILE.retryFloorMs : 10000);
    socket = await welcome(h); await begin(socket, 1);
    assert.equal(jobs.length, 2, "logical cancellation cannot admit a third unresolved job");
  }
  assert.equal(socket.closeCalls, 0);
  await pong(h, 1000); assert.equal(h.transport.state.connected, true);
  const late = new Core(); jobs[0](late); await flush();
  assert.equal(late.disposed, 1); assert.equal(jobs.length, 3, "actual completion admits current waiting restore only");
  const stale = new Core(); jobs[1](stale); await flush(); assert.equal(stale.disposed, 1);
  h.transport.dispose(); const last = new Core(); jobs[2](last); await flush(); assert.equal(last.disposed, 1);
});

function fatalPayload(detail = "terminal ABI mismatch") {
  const text = new TextEncoder().encode(detail), payload = new Uint8Array(4 + text.length);
  payload[0] = 2; payload[2] = 1; payload.set(text, 4);
  return payload;
}

test("connection profile rejects invalid windows", t => {
  for (const value of [0, -1, Infinity, NaN]) assert.throws(() => harness(t, { profile: { responseMs: value } }), /finite and positive/);
});
...
test("two unreleased sockets bound repeated logical retirement; closure alone releases slots", async t => {
  const h = harness(t), first = await healthy(h);
  first.emit("error"); await h.clock.advance(PROFILE.retryFloorMs);
  const second = h.sockets[1]; assert.ok(second);
  second.emit("error"); await h.clock.advance(60000);
  assert.equal(h.sockets.length, 2);
  assert.equal(h.transport.state.status, "resource-wait");
  assert.equal(h.transport.state.unreleasedSockets, 2);
  const waiting = h.transport.connect(); waiting.catch(() => {});
  h.window.emit("online"); h.document.emit("visibilitychange");
  first.bufferedAmount = second.bufferedAmount = 0;
  await h.clock.advance(60000); assert.equal(h.sockets.length, 2);
  first.closed();
  assert.equal(h.sockets.length, 3);
  assert.equal(h.transport.state.unreleasedSockets, 2);
  first.emit("close");
  assert.equal(h.transport.state.unreleasedSockets, 2, "release is exactly once");
  const third = await welcome(h); await waiting;
  third.emit("error"); await h.clock.advance(20000);
  assert.equal(h.sockets.length, 3, "third logical retirement still cannot forge physical closure");
  second.closed(); assert.equal(h.sockets.length, 4);
  h.transport.dispose(); third.closed(); h.sockets[3].closed();
...
test("observed CLOSED state admits only after backoff and releases constructor failures", async t => {
  const h = harness(t), first = await healthy(h);
  first.emit("error"); await h.clock.advance(PROFILE.retryFloorMs);
  const second = h.sockets[1]; second.emit("error");
  first.readyState = Socket.CLOSED;
  const ready = h.transport.connect(); ready.catch(() => {});
  h.window.emit("online"); assert.equal(h.sockets.length, 2, "notification cannot bypass backoff");
  await h.clock.advance(500);
  assert.equal(h.sockets.length, 3);
  assert.equal(h.transport.state.unreleasedSockets, 2);
  h.transport.dispose();
  let attempts = 0;
  const k = harness(t, { webSocketFactory: () => { attempts++; throw new Error("constructor failed"); } });
  const pending = k.transport.connect(); pending.catch(() => {});
...
test("operation cancellation and deadline do not retire a shared connection", async t => {
  const h = harness(t), abort = new AbortController();
  ...
  await h.clock.advance(50); await rejected; await timed;
  assert.equal(h.sockets[0].closeCalls, 0);
  await welcome(h); await survives;
  assert.equal(h.errors.length, 0);
});
...
test("suspect retains attachment/input authority; late PONG clears it without teardown", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core();
  const attached = h.transport.attach(metadata, core);
  await flush(); const attach = await begin(socket, 2, true); await live(socket, attach);
  const record = await attached; h.transport.setActive(record);
  ...
  core.input(new Uint8Array([42])); assert.equal(socket.frames(F.INPUT).length, 1);
  ...
  await h.clock.advance(PROFILE.probeIntervalMs - 1); assert.equal(socket.frames(F.PING).length, count);
  await h.clock.advance(1); assert.equal(socket.frames(F.PING).length, count + 1);
  assert.equal(h.errors.length, 0);
});

test("missing heartbeat starts one hedge without retiring the incumbent", async t => {
  const h = harness(t), incumbent = await healthy(h), core = new Core();
  const attached = h.transport.attach(metadata, core); await flush();
  const attach = await begin(incumbent, 2, true); await live(incumbent, attach);
  const record = await attached; h.transport.setActive(record);
  await h.clock.advance(PROFILE.probeIntervalMs);
...
test("late incumbent heartbeat cancels its hedge without switching", async t => {
  const h = harness(t), incumbent = await healthy(h);
...
test("a ready hedge replaces the incumbent without a disconnected state", async t => {
  const h = harness(t), incumbent = await healthy(h), core = new Core(), statuses = [];
  h.transport.onStatus((status, state) => statuses.push([status, state.connected]));
  const attached = h.transport.attach(metadata, core); await flush();
  const initial = await begin(incumbent, 2, true); await live(incumbent, initial);
  const record = await attached; h.transport.setActive(record);
  await h.clock.advance(PROFILE.probeIntervalMs + PROFILE.hedgeDelayMs);
  const hedge = h.sockets[1];
  hedge.open();
  ...
  assert.ok(statuses.every(([, connected]) => connected), "hedging must not publish a disconnected switchover");
...
test("continuous one-way application traffic and unmatched PONG do not renew response budget", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core();
  const attached = h.transport.attach(metadata, core); await flush();
  const attach = await begin(socket, 2, true); await live(socket, attach); await attached;
  await h.clock.advance(PROFILE.probeIntervalMs);
  const count = socket.frames(F.PING).length;
  const unmatched = new Uint8Array(8); writeUint64LE(unmatched, 0, 1n);
  for (let i = 0; i < 14; i++) {
    await h.clock.advance(1000);
    const payload = new Uint8Array(40); payload[0] = 1;
    writeUint32LE(payload, 4, 8); writeUint32LE(payload, 12, 8);
    writeUint64LE(payload, 16, BigInt(i + 1)); writeGeometry(payload, 32, geometry);
    socket.frame(F.EVENT_BATCH, payload, attachmentFields(attach));
    socket.frame(F.PONG, unmatched, { requestId: 1n }); await flush();
    assert.equal(socket.closeCalls, 0);
  }
  assert.equal(socket.frames(F.ACK).length, 15);
  assert.equal(readUint32LE(socket.frames(F.ACK)[0].payload, 16), 0, "barrier must not duplicate the initial receive grant");
  for (const ack of socket.frames(F.ACK).slice(1)) assert.equal(readUint32LE(ack.payload, 16), 128, "return exactly the native resize-event credit charge");
  assert.equal(socket.frames(F.PING).length, count);
  assert.equal(h.transport.state.unmatchedPongs, 14);
  await h.clock.advance(999); assert.equal(socket.closeCalls, 0);
  await h.clock.advance(1); assert.equal(socket.closeCalls, 1);
...
test("healthy burst/jitter traffic keeps paced probes and FIFO without disconnects", async t => {
  ...
  for (const delay of [40, 110, 600, 1600, 35, 8000, 50, 3000]) {
    ...
    assert.equal(socket.closeCalls, 0); assert.equal(h.transport.state.status, "ready");
  }
  assert.equal(socket.closeCalls, 0);
...
test("control submission stalls retire after their fixed window, never from RTT", async t => {
  ...
  assert.equal(socket.closeCalls, 0); assert.equal(socket.frames(F.PING).length, count);
  await h.clock.advance(1);
  assert.equal(socket.closeCalls, 1);
...
test("pressure released before submission expiry grants full response opportunity", async t => {
  const h = harness(t), socket = await healthy(h);
  socket.bufferedAmount = 4 * 1024 * 1024;
  await h.clock.advance(PROFILE.probeIntervalMs); await h.clock.advance(4000);
  socket.bufferedAmount = 0; await h.clock.advance(100);
  const probe = socket.last(F.PING);
  await h.clock.advance(10000); assert.equal(socket.closeCalls, 0);
  socket.frame(F.PONG, probe.payload, { requestId: probe.requestId });
...
test("fatal ERROR bypasses delayed restore, fences late work, and retries attachment", async t => {
  const h = harness(t), socket = await healthy(h), core = new Core();
  let resolveCore, coreCalls = 0;
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => ++coreCalls === 1 ? new Promise(resolve => { resolveCore = resolve; }) : new Core() });
  ...
  assert.equal(h.transport.state.status, "backoff", "protocol failure schedules retry before callback returns");
  assert.equal(h.errors.length, 1);
  socket.closed(); const late = new Core(); resolveCore(late);
  await flush();
  assert.equal(late.disposed, 1); assert.equal(core.disposed, 0);
  assert.equal(socket.frames(F.ACK).length, 0);
  assert.equal(h.transport.state.queuedApplicationBytes, 0);
  h.window.emit("online"); await h.clock.advance(PROFILE.retryFloorMs);
  const replacement = await welcome(h); const resumed = await begin(replacement, 2, true);
  await live(replacement, resumed); await attached;
  assert.equal(h.transport.state.status, "ready");
});

test("fatal negotiation and malformed ERROR retry without granting peer authority", async t => {
  const h = harness(t);
  const first = h.transport.connect(), second = h.transport.connect();
  const socket = h.sockets[0]; socket.open();
  socket.frame(F.ERROR, fatalPayload(), { requestId: socket.last(F.HELLO).requestId });
  socket.closed();
  assert.equal(h.errors.length, 1); assert.equal(h.transport.state.status, "backoff");
  await h.clock.advance(PROFILE.retryFloorMs); await welcome(h); await first; await second;
  for (const kind of ["flag", "code", "scope", "context", "utf8"]) {
    const k = harness(t), old = await healthy(k), payload = fatalPayload("peer diagnostic"), fields = {};
    ...
    old.frame(F.ERROR, payload, fields); await flush();
    assert.equal(k.errors.length, 1); assert.notEqual(k.errors[0].message, "peer diagnostic");
    assert.equal(k.transport.state.status, "backoff");
    old.closed(); await k.clock.advance(PROFILE.retryFloorMs); await welcome(k);
...
test("nonfatal attachment ERROR remains ordered behind restore", async t => {
  ...
  await flush(); const attach = await begin(socket, 1);
  const payload = fatalPayload("operation failed"); payload[2] = 0;
  socket.frame(F.ERROR, payload, attachmentFields(attach));
  assert.equal(h.errors.length, 0); assert.equal(socket.closeCalls, 0);
  resolveCore(new Core()); await flush(); await rejected;
  assert.equal(h.transport.state.connected, true);
...
test("resume invalidates old probes once; hints and pre-suspension PONG cannot renew", async t => {
  const h = harness(t), socket = await healthy(h);
  await h.clock.advance(PROFILE.probeIntervalMs);
  const old = socket.last(F.PING), samples = h.transport.state._rttSamples.length;
  h.document.hidden = true; h.document.emit("visibilitychange"); h.clock.time += 60000;
  h.document.hidden = false;
  await h.clock.advance(0); // timer-before-visibility ordering
  const fresh = socket.last(F.PING); assert.notEqual(fresh.requestId, old.requestId);
  h.document.emit("visibilitychange"); h.window.emit("online");
  ...
  h.window.emit("online"); h.document.emit("visibilitychange"); assert.equal(socket.closeCalls, 0);
  await h.clock.advance(1); assert.equal(socket.closeCalls, 1);
  assert.equal(h.transport.state.lastOutcome.kind, "response-unavailable");
});
...
test("unannounced scheduling gap and visible freeze grant fresh observation, not timer backlog", async t => {
  const h = harness(t), socket = await healthy(h);
  await h.clock.advance(PROFILE.probeIntervalMs);
  const old = socket.last(F.PING); h.clock.time += 60000;
  socket.frame(F.PONG, old.payload, { requestId: old.requestId }); // message-before-timer ordering
  const fresh = socket.last(F.PING); assert.notEqual(fresh.requestId, old.requestId);
  h.window.emit("online"); assert.equal(socket.last(F.PING).requestId, fresh.requestId);
  await h.clock.advance(0); await pong(h);
  h.document.emit("freeze"); await h.clock.advance(30000); assert.equal(socket.closeCalls, 0);
  h.document.emit("resume"); await pong(h, 500);
  assert.equal(h.transport.state.wsRttLatestMs, 500); assert.equal(h.errors.length, 0);
});

test("hidden cold negotiation receives a fresh HELLO window on actual resume", async t => {
  const h = harness(t); h.document.hidden = true;
  const ready = h.transport.connect(), socket = h.sockets[0]; socket.open();
  await h.clock.advance(60000); assert.equal(socket.closeCalls, 0);
  h.document.hidden = false; h.document.emit("visibilitychange"); await h.clock.advance(9000);
  h.window.emit("online"); h.document.emit("visibilitychange");
  await welcome(h, socket, 500); await ready;
  assert.equal(h.transport.state.negotiationMs, null);
  await pong(h, 500); assert.equal(h.transport.state.wsRttLatestMs, 500); assert.equal(h.sockets.length, 1);
});
...
test("bounded receive queue is released on retirement while restore remains pending", async t => {
  ...
  h.transport.activate({ core, onResize: () => ({ dispose() {} }), onSelectionModeChange: () => ({ dispose() {} }),
    createCore: () => new Promise(resolve => { resolveCore = resolve; }) });
  const attached = h.transport.attach(metadata, core); attached.catch(() => {});
  await flush(); const attach = await begin(socket, 1);
  ...
  assert.equal(h.transport.state.queuedApplicationBytes, 0);
  const late = new Core(); resolveCore(late); await flush();
  assert.equal(late.disposed, 1); assert.equal(socket.frames(F.ACK).length, 0); assert.equal(h.errors.length, 0);
});
