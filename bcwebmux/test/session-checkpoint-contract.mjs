...
assert.ok(WEB_ROOT && ZSTD_COMPRESSOR, "usage: node test/session-checkpoint-contract.mjs WEB_ROOT ZSTD_COMPRESSOR");
const [{ appendCheckpoint, ensureShadow, finishCheckpoint, resetCheckpointTransaction, rollbackShadow }, { crc32c }, { COMPRESSED_FLAG, writeUint32LE }] = await Promise.all([
...
const core = () => ({ disposed: 0, onData() { return { dispose() {} }; }, dispose() { this.disposed++; } });
const original = core(), late = core();
const attempt = { cancel: new AbortController(), retired: false };
const shadowRecord = { active: true, attachmentId: 1n, epoch: 1n, core: original, attempt, eventSeq: 7n, outputOffset: 9n };
const records = new Map([["1", shadowRecord]]);
let create;
const pendingShadow = ensureShadow({ core: original, createCore: () => new Promise(resolve => { create = resolve; }) }, records, shadowRecord, () => {});
attempt.retired = true; attempt.cancel.abort(); resetCheckpointTransaction(shadowRecord);
shadowRecord.attempt = { cancel: new AbortController(), retired: false };
assert.equal(await pendingShadow, false, "retirement releases the await without waiting for core creation");
create(late); await new Promise(resolve => setImmediate(resolve));
assert.equal(late.disposed, 1); assert.equal(shadowRecord.core, original);

const shadow = core();
assert.equal(await ensureShadow({ core: original, createCore: async () => shadow }, records, shadowRecord, () => {}), true);
shadowRecord.eventSeq = 200n; shadowRecord.outputOffset = 300n;
rollbackShadow(shadowRecord);
assert.equal(shadowRecord.core, original); assert.equal(shadow.disposed, 1);
assert.equal(shadowRecord.eventSeq, 7n); assert.equal(shadowRecord.outputOffset, 9n);
...
try {
  ...
  const restoring = { active: true, epoch: 1n, attempt: owner, checkpoint: new Uint8Array([1]), checkpointOffset: 1, checkpointHash: new Uint8Array(32) };
  const finishing = finishCheckpoint(restoring, { payload: new Uint8Array(32) });
  owner.retired = true; owner.cancel.abort(); resetCheckpointTransaction(restoring);
  ...
  resolveDigest(new ArrayBuffer(32)); await new Promise(resolve => setImmediate(resolve));
...
const pool = { active: 0, waiters: [] }, jobs = [];
try {
  crypto.subtle.digest = () => new Promise(resolve => jobs.push(resolve));
  const startDigest = () => {
    const record = { active: true, epoch: 1n, attempt: { cancel: new AbortController(), retired: false },
      restoreCancel: new AbortController(), restoreJobs: { digest: pool },
      checkpoint: new Uint8Array([1]), checkpointOffset: 1, checkpointHash: new Uint8Array(32) };
    return { record, promise: finishCheckpoint(record, { payload: new Uint8Array(32) }) };
  };
  for (let i = 0; i < 5; i++) {
    const pending = startDigest(); resetCheckpointTransaction(pending.record);
    assert.equal(await pending.promise, null);
  }
  assert.equal(jobs.length, 2); assert.equal(pool.active, 2); assert.equal(pool.waiters.length, 0);
  const current = startDigest(); assert.equal(pool.waiters.length, 1);
  jobs[0](new ArrayBuffer(32)); await new Promise(resolve => setImmediate(resolve));
  assert.equal(jobs.length, 3); assert.equal(pool.active, 2);
  resetCheckpointTransaction(current.record); assert.equal(await current.promise, null);
  jobs[1](new ArrayBuffer(32)); jobs[2](new ArrayBuffer(32));
  await new Promise(resolve => setImmediate(resolve));
  assert.equal(pool.active, 0); assert.equal(pool.waiters.length, 0);
} finally { crypto.subtle.digest = digest; }
