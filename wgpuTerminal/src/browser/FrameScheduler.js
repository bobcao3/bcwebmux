...
export class FrameScheduler {
  ...
  schedule(immediate = false) {
    ...
    this.coreDirty = true;
    if (this.blocked) { this._cancel(); return; }
    if (immediate && (!this._submitted || this._now() - this._submittedAt >= 1000 / 60)) this._flush();
    this._request();
  }
  ...
  suspend() { this._suspended = true; this._cancel(); }
  recover() {
    if (this._disposed) return;
    this._suspended = false;
    this.schedule();
  ...
  dispose() { this._disposed = true; this._cancel(); this._terminal = null; }
  ...
  _flush() {
    ...
    try {
      ...
      if (this.blocked) { this.coreDirty = true; this._cancel(); return result; }
  ...
  _request() {
    ...
    this._frame = this._raf(() => {
      ...
      if (backend && opportunity !== null && Number.isFinite(opportunity)) {
        backend.presentationOpportunityMs = backend.presentationOpportunityMs === null
          ? opportunity
