...
export class PointerController {
  ...
  sendMouse(event, action, button, anyButtonPressed = event.buttons !== 0) {
    ...
    return this.mouse(action, button, pointerModifiers(event), x, y, Boolean(anyButtonPressed));
  }
        ...
        if (event.pointerType === "touch") {
          if (this.isTerminalPointer(event)) {
            ...
            candidate.longPressTimer = setTimeout(() => {
              ...
              if (
                this.enterSelectionMode(candidate.startX, candidate.startY, x, y) &&
      ...
      (event) => {
        ...
        if (event.pointerType === "touch") {
          ...
          if (candidate?.pointerId === event.pointerId) {
            ...
            candidate.suppress =
              candidate.moved || candidate.duration >= this.touchLongPressThreshold;
            if (candidate.moved) {
