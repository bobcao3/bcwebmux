...
async function pixels(source) {
  ...
  if (source.compression) {
    ...
    try {
      while (true) {
        ...
        length += value.length;
        if (
          length > (source.format === 100 ? 32 * 1024 * 1024 : source.width * source.height * 4)
        ) {
          await reader.cancel();
  ...
  if (format === 100) {
    if (source.pngSize && bytes.length !== source.pngSize)
      throw new Error("invalid graphics PNG length");
    ...
    try {
      if (bitmap.width !== width || bitmap.height !== height)
        throw new Error("invalid graphics PNG dimensions");
...
export class GraphicsScene {
  ...
  update(packet) {
    ...
    for (let i = 0; i < resources.byteLength / 44; i++) {
      ...
      if (retained) {
        ...
        this.sources.set(key, {
          width: resources.getUint32(o + 16, true),
          height: resources.getUint32(o + 20, true),
          texture: null,
        });
      }
    }
    for (const value of previous.values()) {
      if (value.texture) {
        this.decodedBytes -= value.width * value.height * 4;
        this.backend.destroyGraphicsTexture(value.texture);
      }
    }
    this.draws = [];
    ...
    for (let i = 0; i < draws.byteLength / 48; i++) {
      const o = i * 48;
      this.draws.push({
        key: keys[draws.getUint32(o, true)],
        z: draws.getInt32(o + 4, true),
        x: draws.getInt32(o + 8, true),
        y: draws.getInt32(o + 12, true),
        width: draws.getUint32(o + 16, true),
        height: draws.getUint32(o + 20, true),
        sourceX: draws.getUint32(o + 24, true),
        sourceY: draws.getUint32(o + 28, true),
        sourceWidth: draws.getUint32(o + 32, true),
        sourceHeight: draws.getUint32(o + 36, true),
        offsetX: draws.getUint32(o + 40, true),
        offsetY: draws.getUint32(o + 44, true),
      });
    }
    const queue = [];
    for (let i = 0; i < keys.length; i++) {
      ...
      const o = i * 44;
      const width = resources.getUint32(o + 16, true),
        height = resources.getUint32(o + 20, true);
      ...
      queue.push({
        ...
        width,
        height,
        format: resources.getUint32(o + 24, true),
        compression: resources.getUint32(o + 28, true),
        pngSize: resources.getUint32(o + 32, true),
        bytes: Uint8Array.from(packet.graphicsBytes[i]),
      });
    }
    const worker = async () => {
