...
export function resolveWasmFontUrls(wasmUrl, values) {
  ...
  const urls =
    values == null ? DEFAULT_WASM_FONT_FILES.map((file) => `fonts/${file}`) : [...values];
  if (
    urls.length !== 4 ||
    urls.some((value) => typeof value !== "string" && !(value instanceof URL))
  ) {
...
function fetchFont(url) {
  ...
  if (!pending) {
    pending = fetch(url).then(async (response) => {
      if (!response.ok)
        throw new Error(`terminal font request failed (${response.status}): ${url}`);
