    ...
    emit(...args) {
      for (const listener of [...listeners]) {
        try {
          ...
          console.error("session controller listener failed", error);
        }
      }
    },
    clear() {
...
function activityTime(metadata) {
  return Number(metadata.lastActivityMs ?? metadata.last_activity_ms ?? 0) || 0;
}
...
function sameObservableMetadata(a, b) {
  ...
  return (
    Object.is(
      a?.generation == null ? null : String(a.generation),
      b?.generation == null ? null : String(b.generation),
    ) &&
    Object.is(a?.name == null ? null : String(a.name), b?.name == null ? null : String(b.name)) &&
    Object.is(
      a?.title == null ? null : String(a.title),
      b?.title == null ? null : String(b.title),
    ) &&
    Object.is(
      a?.state == null ? null : String(a.state),
      b?.state == null ? null : String(b.state),
    ) &&
    ...
    Object.is(
      aController?.attachmentId == null ? null : String(aController.attachmentId),
      bController?.attachmentId == null ? null : String(bController.attachmentId),
    ) &&
    Object.is(
      aController?.leaseEpoch == null ? null : String(aController.leaseEpoch),
      bController?.leaseEpoch == null ? null : String(bController.leaseEpoch),
    ) &&
...
export class SessionController {
  ...
  constructor(options = {}) {
    ...
    this.#coreLimit = Number.isFinite(coreLimit)
      ? Math.max(1, Math.floor(coreLimit))
      : DEFAULT_CORE_LIMIT;
    if (!this.#terminal || !this.#api || !this.#transport)
      throw new TypeError("SessionController requires terminal, api, and transport");
    this.#subscriptions.push(
      this.#transport.onStatus?.((label, state) => {
        this.#syncPointers();
  ...
  onChange(listener) {
    return this.#events.change.on(listener);
  }
  onActiveChange(listener) {
    return this.#events.active.on(listener);
  }
  onTitleChange(listener) {
    return this.#events.title.on(listener);
  }
  onBell(listener) {
    return this.#events.bell.on(listener);
  }
  onNotification(listener) {
    return this.#events.notification.on(listener);
  }
  onError(listener) {
    return this.#events.error.on(listener);
  }
  onStatus(listener) {
    return this.#events.status.on(listener);
  }
  on(event, listener) {
    return (
      this.#events[event]?.on(listener) ??
      (() => {
        throw new Error(`unknown controller event: ${event}`);
      })()
    );
  }
  ...
  get activeSession() {
    return this.#activeId ? (this.#metadata.get(this.#activeId) ?? null) : null;
  }
  get active() {
  ...
  get activeAttachment() {
    this.#syncPointers();
    return this.#activeId ? (this.#cores.get(this.#activeId)?.attachment ?? null) : null;
  }
  get activeCore() {
    this.#syncPointers();
    return this.#activeId ? (this.#cores.get(this.#activeId)?.core ?? null) : null;
  }
  get pendingSwitches() {
    return [...this.#pendingSwitches.keys()];
  }
  get coreCount() {
  ...
  isAttached(id) {
    const attachment = this.#cores.get(String(id))?.attachment;
    return Boolean(attachment?.active && attachment?.live);
  }
  isController(id) {
    return Boolean(this.#cores.get(String(id))?.attachment?.controller);
  }

  start() {
    if (this.#disposed) return Promise.reject(new Error("session controller is disposed"));
  ...
  async #start() {
    ...
    const [info, list] = await Promise.all([
      this.#api.info(),
      this.#api.list(),
      this.#transport.connect({ signal: this.#lifetime.signal }),
    ]);
    ...
    this.#setStorageScope(
      this.#serverInfo.serverInstance ?? this.#serverInfo.server_instance ?? "server",
      this.#serverInfo.principal ?? "principal",
    );
    this.#replaceList(list);
    let selected = this.#readLastSession();
    if (!selected || !this.#metadata.has(selected))
  ...
  #checkServerInstance() {
    ...
    if (
      ...
      !this.#serverInfo ||
      !instance ||
      instance === this.#serverInfo.serverInstance
    )
      return false;
    if (!this.#restartPromise) {
      ...
      operation
        .catch((error) => this.#reportRefreshError(error))
        .finally(() => {
  ...
  async #reconcileServer(instance) {
    ...
    if (info?.protocol !== "bcw.sessions" || info.serverInstance !== instance)
      throw new Error("server instance changed during session refresh");
    const list = await this.#api.list();
    ...
    if (!selected || !this.#metadata.has(selected) || selected === missingActive) {
      selected = this.sessions
        .filter((item) => item.id !== missingActive && item.state === "running")
        .sort((a, b) => activityTime(b) - activityTime(a))[0]?.id;
    }
    try {
  ...
  async switchTo(id) {
    ...
    if (this.#activeId === key && entry?.attachment && !entry.attachment.active)
  ...
  async #switchNow(key) {
    ...
    try {
      ...
      if (record.live === false)
        throw new Error("session attachment did not reach the live barrier");
    } catch (error) {
      this.#syncEntry(entry, record);
  ...
  async #waitForLive(key, entry) {
    ...
    return new Promise((resolve, reject) => {
      ...
      timer = setTimeout(
        () => finish(new Error("session switch readiness deadline exceeded")),
        30000,
      );
      check();
    });
  ...
  async #ensureEntry(metadata) {
    ...
    if (
      ...
      entry.generation &&
      metadata.generation &&
      entry.generation !== String(metadata.generation)
    ) {
    ...
    if (!entry.core || entry.core.disposed) {
      ...
      const inUse = [...this.#cores.values()].some(
        (item) => item !== entry && item.core === hostCore,
      );
      const core =
        !inUse && hostCore && !hostCore.disposed ? hostCore : await this.#terminal.createCore();
  ...
  #syncPointers() {
    for (const entry of this.#cores.values())
      if (entry.attachment) this.#syncEntry(entry, entry.attachment);
  }
  ...
  #detach(entry) {
    ...
    if (entry.attachment) {
      try {
        this.#transport.detach(entry.attachment);
      } catch (error) {
    ...
    this.#events.change.emit({
      type: "detached",
      session: this.#metadata.get(entry.id),
      controller: this,
    });
  }
  ...
  #touch(entry) {
    entry.used = Date.now();
    if (this.#cores.get(entry.id) === entry) {
      this.#cores.delete(entry.id);
      this.#cores.set(entry.id, entry);
    }
  }

  #makeCoreRoom() {
    while (this.#cores.size >= this.#coreLimit) {
      const candidate = [...this.#cores.values()].find(
        (entry) => entry.id !== this.#activeId && !entry.attachment,
      );
  ...
  #evict() {
    while (this.#cores.size > this.#coreLimit) {
      const candidate = [...this.#cores.values()].find(
        (entry) =>
          entry.id !== this.#activeId && !this.#pendingSwitches.has(entry.id) && !entry.attachment,
      );
      if (!candidate) return;
      this.#closeEntry(candidate);
    }
  ...
  #replaceList(payload, requestedAtRevision = null) {
    ...
    if (
      ...
      this.#listRevision > requestedAtRevision &&
      incomingRevision <= this.#listRevision
    )
      return;
    if (Number.isFinite(incomingRevision))
      this.#listRevision = Math.max(this.#listRevision, incomingRevision);
    ...
    for (const value of values) {
      ...
      const hasNewActivity = Boolean(
        previous &&
        metadata.id !== this.#activeId &&
        cursor(metadata.outputOffset) > cursor(previous.outputOffset),
      );
    ...
    for (const [id, entry] of [...this.#cores])
      if (
        !seen.has(id) &&
        id !== this.#activeId &&
        !this.#pendingSwitches.has(id) &&
        (this.#activeId || entry.core !== this.#terminal.core)
      ) {
        this.#closeEntry(entry);
        changed = true;
      }
    for (const id of [...this.#metadata.keys()])
      if (!seen.has(id) && id !== this.#activeId && !this.#pendingSwitches.has(id)) {
  ...
  #setStorageScope(serverInstance, principal) {
    const server = encodeURIComponent(String(serverInstance || "server"));
    const user = encodeURIComponent(String(principal || "principal"));
    this.#storageKey = `bcwebmux.last-session:${STORAGE_VERSION}:${server}:${user}`;
    this.#storageDenied = false;
  }
  ...
  #reportError(error, global) {
    const value = error instanceof Error ? error : new Error(String(error ?? "session error"));
    this.#events.error.emit(value, global, this);
    this.#events.change.emit({ type: "error", error: value, global, controller: this });
  }
  ...
  async rename(id, name) {
    const key = String(id);
    const result = await this.#api.rename(key, String(name));
    const metadata = this.#putMetadata(result ?? this.#metadata.get(key));
    this.#events.change.emit({ type: "renamed", session: metadata, controller: this });
    return metadata;
  }

  async terminate(id) {
    const key = String(id);
    const result = await this.#api.terminate(key);
    const metadata = this.#putMetadata(result ?? this.#metadata.get(key));
    this.#events.change.emit({ type: "terminated", session: metadata, controller: this });
    return metadata;
  }
  ...
  async delete(id) {
    ...
    try {
      ...
      if (wasActive && !this.#activeId)
        await this.switchTo(key).catch((rollbackError) => this.#reportError(rollbackError, true));
      throw error;
    }
    ...
    return (
      this.sessions
        ...
        .sort((a, b) => {
          ...
          return running || activityTime(b) - activityTime(a);
        })[0] ?? null
    );
  }
  ...
  async claim(id = this.#activeId) {
    const key = String(id);
    if (this.#activeId !== key) await this.switchTo(key);
    const attachment = this.#cores.get(key)?.attachment;
    if (!attachment?.active || !attachment.live)
      throw new Error("session attachment is reconnecting");
    const result = this.#transport.claimControl(attachment);
    if (result === false) throw new Error("session attachment is reconnecting");
    this.#events.change.emit({ type: "claim", session: this.#metadata.get(key), controller: this });
    return result;
  }
  ...
  #putMetadata(value) {
    ...
    const current = this.#metadata.get(metadata.id);
    if (current) {
      if (revision > 0 && Number.isFinite(revision) && revision < Number(current.revision ?? 0))
        return current;
