...
function migrateFontFamilies(families) {
  ...
  const symbolsIndex = families.findIndex(
    (family) => family.toLowerCase() === "noto sans symbols 2",
  );
...
const DEFAULT_CUSTOM = {
  ...
  name: "Custom",
  description: "Your saved profile.",
};
...
function customAnsi(custom) {
  return DEFAULT_CUSTOM.ansi.map((color, index) =>
    index === 0 ? custom.background : index === 15 ? custom.foreground : color,
  );
}
...
function loadSettings() {
  const fallback = {
    ...
    fontFallbacks: Object.fromEntries(
      Object.keys(FONT_OPTIONS).map((id) => [
        id,
        normalizeFontFamilies(undefined, DEFAULT_FONT_FALLBACKS[id]),
      ]),
    ),
  ...
  try {
    ...
    for (const field of COLOR_FIELDS)
      custom[field] = normalizeColor(saved.custom?.[field], custom[field]);
    custom.ansi = customAnsi(custom);
    const selected =
      saved.selected === "custom" || BUILTIN_PROFILES[saved.selected]
        ? saved.selected
        : DEFAULT_PROFILE;
    const fontFamily = Object.hasOwn(FONT_OPTIONS, saved.fontFamily)
      ? saved.fontFamily
    ...
    const ligatures =
      typeof saved.ligatures === "boolean" ? saved.ligatures : DEFAULT_SETTINGS.ligatures;
    ...
    const perfMode = ["off", "simple", "detailed"].includes(saved.perfMode)
      ? saved.perfMode
      : DEFAULT_SETTINGS.perfMode;
    const renderer =
      FONT_OPTIONS[fontFamily].canvasOnly || saved.renderer === "kb-canvas"
        ? "canvas"
        : ["kb-stb", "canvas"].includes(saved.renderer)
          ? saved.renderer
          : DEFAULT_SETTINGS.renderer;
    const migrateFontFallbacks = !Object.hasOwn(saved, "fontFallbackVersion");
    const fontFallbacks = Object.fromEntries(
      Object.keys(FONT_OPTIONS).map((id) => {
        const fallbacks = normalizeFontFamilies(
          saved.fontFallbacks?.[id],
          DEFAULT_FONT_FALLBACKS[id],
        );
  ...
  try {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        fontFallbacks: Object.fromEntries(
          Object.entries(settings.fontFallbacks).map(([id, fallbacks]) => [id, [...fallbacks]]),
        ),
...
export function initializeSettings() {
  ...
  const activate = (id, persist = true) => {
    ...
    profileList.querySelectorAll("input").forEach((input) => {
  ...
  }
  {
    ...
    for (const color of [
      ...
      settings.custom.foreground,
      settings.custom.accent,
      settings.custom.success,
      settings.custom.danger,
    ]) {
      const swatch = document.createElement("i");
      swatch.style.background = color;
      swatches.append(swatch);
  ...
  openButton.addEventListener("click", () => {
    ...
    fontSettingsForm.elements.fontFallbacks.value =
      settings.fontFallbacks[settings.fontFamily].join("\n");
    syncFontSize();
  ...
  tablist.addEventListener("keydown", (event) => {
    ...
    const next =
      tabs[(tabs.indexOf(document.activeElement) + direction + tabs.length) % tabs.length];
  ...
  customForm.addEventListener("submit", (event) => {
    event.preventDefault();
    for (const field of COLOR_FIELDS)
      settings.custom[field] = normalizeColor(
        customForm.elements[field].value,
        DEFAULT_CUSTOM[field],
      );
    settings.custom.ansi = customAnsi(settings.custom);
    const customOption = profileList
    ...
    [
      ...
      customOption.querySelectorAll(".profile-swatches i")[index].style.background = color;
    });
    activate("custom");
  });
  fontSettingsForm.elements.fontFamily.value = settings.fontFamily;
  ...
  fontSettingsForm.addEventListener("change", async (event) => {
    ...
    if (renderer !== settings.renderer) {
      ...
      try {
        ...
        fontSettingsForm.elements.fontFallbacks.value =
          settings.fontFallbacks[previousFontFamily].join("\n");
        fontSettingsForm.elements.renderer.value = settings.renderer;
        syncRendererControl();
        return;
      } finally {
  ...
  return {
    ...
    get glyphCacheMaxMiB() {
      return settings.glyphCacheMaxMiB;
    },
    get perfMode() {
    ...
    setOnFontChange(callback) {
      onFontChange = callback || (() => {});
    },
    setOnGrainChange(callback) {
      onGrainChange = callback || (() => {});
    },
    setOnPerfChange(callback) {
      onPerfChange = callback || (() => {});
    },
    setOnRendererChange(callback) {
