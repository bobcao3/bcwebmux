...
export const CONNECTION_PROFILE = Object.freeze({
  connectMs: 10000,
  helloMs: 10000,
  controlSubmitMs: 5000,
  responseMs: 15000,
  probeIntervalMs: 1000,
  hedgeDelayMs: 1000,
  schedulingGapMs: 5000,
  retryFloorMs: 500,
  retryCeilingMs: 10000,
});
const textDecoder = new TextDecoder("utf-8", { fatal: true });
const OUTPUT_STREAM_PREAMBLE = new TextEncoder().encode(
...
export class SessionTransport {
  ...
  #waiters = new Set();
  get #socket() {
    return this.#attempt?.socket ?? null;
  }
  ...
  constructor(options = {}) {
    ...
    this.#clientId = options.clientInstanceId
      ? uuidBytes(options.clientInstanceId)
      : stableClientId();
    ...
    for (const value of Object.values(this.#profile)) {
      if (!Number.isFinite(value) || value <= 0)
        throw new TypeError("connection profile windows must be finite and positive");
    }
    if (this.#profile.retryFloorMs > this.#profile.retryCeilingMs)
      throw new TypeError("invalid retry backoff range");
    this.#window = options.window ?? globalThis.window;
    this.#document = options.document ?? globalThis.document;
    this.#wakeListener = () => this.#wake();
    this.#freezeListener = () => {
      this.#frozen = true;
      this.#suspend(this.#attempt);
    };
    this.#resumeListener = () => {
      this.#frozen = false;
      this.#wake();
  ...
  connect(options = {}) {
    ...
    const promise = new Promise((resolve, reject) => {
      const waiter = {
        settle: (error, failed = error !== undefined) => {
      ...
      if (options.timeoutMs !== undefined)
        waiter.timer = this.#clock.setTimeout(
          () => waiter.settle(new Error("connection operation deadline exceeded")),
          options.timeoutMs,
        );
    });
    this.#admit();
  ...
  #admit() {
    ...
    const attempt = {
      ...
      lastTick: now,
      timer: 0,
      suspended: this.#frozen || !!this.#document?.hidden,
      helloRequestId: 0n,
      sendSequence: 0n,
      serverSequence: 0n,
      probe: null,
  ...
  #listenHedge(hedge) {
    ...
    const open = () => {
      ...
      const frame = encodeFrame({
        ...
        payload,
        requestId: hedge.helloRequestId,
        attachmentId: 0n,
        attachmentEpoch: 0n,
        sessionId: ZERO_SESSION_ID,
    ...
    const message = (event) => {
      ...
      try {
        ...
        if (frame.connectionSequence !== hedge.serverSequence + 1n)
          throw new Error("server connection sequence gap");
  ...
  #cancelHedge(hedge, failed = false) {
    ...
    if (failed) {
      const delay = Math.min(
        this.#profile.retryCeilingMs,
        this.#profile.retryFloorMs * 2 ** Math.min(this.#hedgeFailures++, 16),
      );
  ...
  async attach(metadata, core, options = {}) {
    if (!metadata?.id || !metadata?.generation || !core)
      throw new TypeError("attach requires session metadata and core");
    do {
    ...
    if (this.#records.size >= this.#attempt.maxAttachments)
      throw new Error("connection attachment limit reached");
    if (this.#records.size + this.#attempt.canceledAttaches.size >= 64)
      throw new Error("pending attachment cleanup limit reached");
    const record = {
      metadata,
  ...
  setActive(record) {
    ...
    if (record && !this.#records.has(record.attachmentId.toString()))
      throw new Error("attachment is not owned by this transport");
    if (this.#activeRecord !== record && this.#terminal?.selectionMode)
      this.#terminal.exitSelectionMode({ restoreFocus: false });
    this.#flushFrozen(this.#activeRecord);
    this.#activeRecord = record;
    if (record?.live && this.#terminal && this.#terminal.core !== record.core)
      this.#terminal.attachCore(record.core);
  ...
  onAttachmentChanged(listener) {
    return this.#attachmentEmitter.event(listener);
  }
  onSessionChanged(listener) {
    return this.#sessionChangedEmitter.event(listener);
  }
  onError(listener) {
  ...
  get clientInstanceId() {
    return bytesUuid(this.#clientId);
  }
  ...
  dispose() {
    ...
    for (const disposable of this.#terminalDisposables.splice(0)) disposable.dispose?.();
    for (const record of [...this.#records.values()])
      this.#removeRecord(record, new Error("session transport disposed"));
  ...
  #socketMessage(attempt, event) {
    ...
    this.#state.rxWireBytes += bytes.byteLength;
    const frame = decodeFrame(bytes);
    if (frame.connectionSequence !== attempt.serverSequence + 1n)
      throw new Error("server connection sequence gap");
    ...
    if (frame.type === FrameType.ERROR && isConnectionFrame(frame)) {
      ...
      if (
        frame.requestId !== 0n &&
        frame.requestId !== attempt.helloRequestId &&
        frame.requestId !== attempt.probe?.nonce
      )
        throw new Error("invalid connection ERROR context");
    ...
    if (attempt.phase !== "ready" && frame.type !== FrameType.ERROR)
      throw new Error("WELCOME required before session frames");
    if (frame.type === FrameType.PONG) {
      if (!isConnectionFrame(frame) || frame.requestId === 0n || frame.payload.byteLength !== 8)
    ...
    if (
      attempt.queueBytes + bytes.byteLength > this.#applicationBudget() ||
      attempt.queue.length - attempt.queueHead >= MAX_APPLICATION_FRAMES
    ) {
      return this.#retire(
        attempt,
        new Error("application receive queue limit exceeded"),
        "receive-overload",
      );
    }
  ...
  async #drainApplication(attempt) {
    ...
    try {
      for (
        let count = 0;
        this.#current(attempt) && attempt.queueHead < attempt.queue.length && count < 32;
        count++
      ) {
  ...
  async #applyFrame(socket, frame) {
    ...
    if (!this.#state.connected && frame.type === FrameType.ERROR)
      return this.#protocolError(null, frame);
    ...
    if (
      frame.attachmentId &&
      !record &&
      this.#staleAttachmentIds.has(frame.attachmentId.toString())
    ) {
      ...
      if (canceled && frame.type === FrameType.ATTACH_BEGIN) {
        if (
          !equalBytes(frame.sessionId, canceled.sessionId) ||
          frame.requestId !== canceled.attachRequestId ||
          frame.attachmentEpoch === 0n ||
          frame.payload.byteLength !== 80 ||
          frame.payload[16] > 2 ||
          frame.payload[18] !== 0 ||
          frame.payload[19] !== 0 ||
          frame.payload.subarray(36, 48).some((byte) => byte !== 0)
        )
          throw new Error("invalid canceled ATTACH_BEGIN");
      ...
      } else if (canceled && frame.type === FrameType.ERROR)
        this.#attempt.canceledAttaches.delete(frame.attachmentId.toString());
      return;
    }
    switch (frame.type) {
      case FrameType.ATTACH_BEGIN:
        return this.#attachBegin(record, frame, socket);
      case FrameType.CHECKPOINT_BEGIN:
        return this.#checkpointBegin(record, frame);
      case FrameType.CHECKPOINT_CHUNK:
        return this.#checkpointChunk(record, frame);
      case FrameType.CHECKPOINT_END:
        return this.#checkpointEnd(record, frame, socket);
      case FrameType.EVENT_BATCH:
        return this.#event(record, frame);
      case FrameType.LIVE_BARRIER:
        return this.#live(record, frame);
      case FrameType.LEASE_CHANGED:
        return this.#lease(record, frame);
      case FrameType.CANONICAL_RESIZE:
        return this.#canonicalResize(record, frame);
      case FrameType.INPUT_ACK:
        return this.#inputAck(record, frame);
      case FrameType.RESYNC_REQUIRED:
        return this.#resync(record);
      case FrameType.EXITED:
        return this.#exited(record, frame);
      case FrameType.SESSION_CHANGED:
        if (!isConnectionFrame(frame) || frame.requestId !== 0n || frame.payload.byteLength !== 8)
          throw new Error("invalid SESSION_CHANGED");
        this.#sessionChangedEmitter.emit(readUint64LE(frame.payload, 0));
        return;
      case FrameType.ERROR:
        return this.#protocolError(record, frame);
      default:
  ...
  #welcome(frame) {
    ...
    this.#state.negotiationMs =
      attempt.helloSuspended || this.#document?.hidden
        ? null
        : this.#clock.now() - attempt.phaseStarted;
  ...
  #validateWelcome(attempt, frame) {
    if (
      ...
      frame.payload.byteLength !== 88 ||
      !isConnectionFrame(frame) ||
      attempt.helloRequestId === 0n ||
      frame.requestId !== attempt.helloRequestId
    )
      throw new Error("invalid WELCOME");
    if (!equalBytes(frame.payload.subarray(16, 48), ABI_DIGEST))
      throw new Error("terminal ABI mismatch; reload/update required");
    if (!validWelcomeCapabilities(frame.payload, MAX_FRAME_LENGTH, INITIAL_CREDIT))
      throw new Error("invalid WELCOME capabilities");
  }
  ...
  async #attachBegin(record, frame, socket) {
    ...
    if (
      ...
      frame.requestId !== record.attachRequestId ||
      frame.payload[18] !== 0 ||
      frame.payload[19] !== 0 ||
      frame.payload.subarray(36, 48).some((byte) => byte !== 0)
    )
      throw new Error("invalid ATTACH_BEGIN");
    ...
    const replayMutatesCore =
      mode === 1 || (mode === 0 && (record.eventSeq !== 0n || record.outputOffset !== 0n));
    if (record.preserveCore && replayMutatesCore) {
      if (this.#terminal) record.restoreGeneration = previousGeneration;
      try {
        if (
          !(await ensureShadow(this.#terminal, this.#records, record, (data) =>
            this.#sendInput(record, data),
          ))
        )
          return;
      } catch (error) {
    ...
    if (mode === 0) {
      ...
      record.core.setReplayMode(true);
      if (record.eventSeq === 0n && record.outputOffset === 0n)
        record.core.resizeCanonical(replayGeometry);
    } else if (mode !== 1) {
      throw new Error("invalid attach mode");
    }
    if (record.active && this.#current(record.attempt))
      this.#attachmentEmitter.emit(record, "attaching");
  }
  ...
  #event(record, frame) {
    ...
    if (record === this.#activeRecord && this.#terminal?.selectionMode) {
      ...
      if (record.frozenBytes > MAX_FROZEN_BYTES)
        this.#terminal.exitSelectionMode({ restoreFocus: false });
      return;
    }
  ...
  #applyEvent(record, flags, payload) {
    ...
    if (
      eventSeq !== record.eventSeq + 1n ||
      outputOffset !== record.outputOffset ||
      wireLength !== payload.byteLength - 32
    )
      throw new Error("terminal event gap");
    ...
    if (kind === 0) {
      ...
      if (record === this.#activeRecord && this.#terminal?.core === record.core)
      ...
      if (flags !== 0 || rawLength !== 8 || body.byteLength !== 8)
        throw new Error("invalid resize event");
      record.core.resizeCanonical(readGeometry(body, 0));
    } else if (kind === 2) {
      if (flags !== 0 || rawLength !== 12 || body.byteLength !== 12)
        throw new Error("invalid exit event");
      record.metadata.exitStatus = new DataView(
        body.buffer,
        body.byteOffset,
        body.byteLength,
      ).getInt32(0, true);
    ...
    if (kind !== 0 && record.active && this.#current(record.attempt))
      this.#attachmentEmitter.emit(record, kind === 1 ? "resize" : "exit");
  }
  ...
  #live(record, frame) {
    ...
    if (eventSeq !== record.eventSeq || outputOffset !== record.outputOffset)
      throw new Error("live barrier cursor mismatch");
    record.core.setReplayMode(false);
    if (record.previousCore && (record.previousWasHostActive || record === this.#activeRecord))
      this.#terminal.attachCore(record.core);
    ...
    record.resolve = null;
    record.reject = null;
    if (record === this.#activeRecord && record.claim && !record.controller)
      this.claimControl(record);
    if (record === this.#activeRecord && record.controller && this.#pendingSize)
      this.#sendResize(this.#pendingSize);
    this.#markHealthy();
  ...
  #lease(record, frame) {
    ...
    this.#markHealthy();
    if (record.active && this.#current(record.attempt))
      this.#attachmentEmitter.emit(record, "lease");
  }
  ...
  #resync(record) {
    ...
    this.#sendAttach(record);
    if (record.active && this.#current(record.attempt))
      this.#attachmentEmitter.emit(record, "resync");
  }
  ...
  #validateError(record, frame) {
    if (
      ...
      (!record && !isConnectionFrame(frame)) ||
      (record && !equalBytes(frame.sessionId, record.sessionId)) ||
      (record && record.epoch !== 0n && frame.attachmentEpoch !== record.epoch) ||
      (record &&
        !record.bindingLive &&
        record.attachRequestId !== 0n &&
        frame.requestId !== record.attachRequestId)
    ...
    const code = readUint16LE(frame.payload, 0),
      fatal = readUint16LE(frame.payload, 2);
  ...
  #sendAttach(record) {
    ...
    this.#send(
      ...
      payload,
      record,
      (record.attachRequestId = this.#nextRequestId()),
      0n,
    );
  }
  ...
  #sendInput(record, data) {
    if (
      ...
      return;
    if (
      data.byteLength + 88 > MAX_FRAME_LENGTH ||
      this.#attempt?.sends.length ||
      this.#attempt?.controls.length ||
      this.#socket?.bufferedAmount + data.byteLength + 88 > MAX_BUFFERED_AMOUNT - CONTROL_RESERVE
    ) {
      this.#errorEmitter.emit(
        new Error("terminal input dropped because websocket is backpressured"),
      );
      return;
    }
  ...
  #sendResize(size) {
    ...
    if (
      geometry?.cols === size.cols &&
      geometry?.rows === size.rows &&
      geometry?.cellWidthPx === cellWidthPx &&
      geometry?.cellHeightPx === cellHeightPx
    )
      return;
  ...
  #flushCredits() {
    ...
    for (const record of this.#records.values())
      if (record.pendingCredit && record.epoch) this.#sendCredit(record, 0);
  }

  #applicationBudget() {
    return Math.min(
      MAX_APPLICATION_BYTES,
      INITIAL_CREDIT * (this.#attempt?.maxAttachments ?? 8) + MAX_FRAME_LENGTH,
    );
  }
  ...
  #send(type, payload, record, requestId = 0n, epoch = record?.epoch ?? 0n) {
    ...
    if (
      !this.#current(attempt) ||
      attempt.socket.readyState !== 1 ||
      (record && (!record.active || record.attempt !== attempt))
    )
      return false;
    ...
    const item = {
      ...
      bytes: 64 + payload.byteLength,
      created: this.#clock.now(),
    };
    const limit = control ? MAX_BUFFERED_AMOUNT : MAX_BUFFERED_AMOUNT - CONTROL_RESERVE;
    if (
      !(control ? attempt.controls.length : attempt.controls.length + attempt.sends.length) &&
      attempt.socket.bufferedAmount + item.bytes <= limit
    )
      return this.#submit(attempt, item);
    // Never retain input whose delivery would be uncertain across retirement.
    if (type === FrameType.INPUT) return false;
    if (
      (control ? attempt.controlBytes : attempt.sendBytes) + item.bytes >
      (control ? CONTROL_RESERVE : MAX_SEND_QUEUE_BYTES)
    ) {
    ...
    if (control) attempt.controlBytes += item.bytes;
    else attempt.sendBytes += item.bytes;
  ...
  #submit(attempt, item) {
    ...
    const frame = encodeFrame({
      ...item,
      connectionSequence: ++attempt.sendSequence,
    });
    try {
    ...
    if (item.type === FrameType.PING && attempt.probe?.nonce === item.requestId)
      attempt.probe.submitted = this.#clock.now();
  ...
  #ping() {
    const attempt = this.#attempt;
    if (!this.#state.connected || attempt.probe || attempt.suspended || this.#document?.hidden)
      return;
  ...
  #markHealthy() {
    ...
    const controlReady = this.#activeRecord
      ? !this.#activeRecord.claim || this.#activeRecord.controller
      : ![...this.#records.values()].some((record) => record.claim && !record.controller);
  ...
  #wake() {
    ...
    if (!this.#document?.hidden && !this.#frozen && this.#attempt.socket.readyState !== 1) {
      this.#retire(
        this.#attempt,
        new Error("session socket unavailable after resume"),
        "socket-unavailable",
      );
    }
  }
  ...
  #armTick(attempt) {
    ...
    if (!attempt.suspended) {
      if (attempt.phase !== "ready")
        deadline = Math.min(
          deadline,
          attempt.phaseStarted +
            (attempt.phase === "connecting" ? this.#profile.connectMs : this.#profile.helloMs),
        );
      else if (probe)
        deadline = Math.min(
          deadline,
          (probe.submitted ?? probe.created) +
            (probe.submitted === null ? this.#profile.controlSubmitMs : this.#profile.responseMs),
        );
  ...
  #tick(attempt) {
    ...
    if (!attempt.suspended) {
      if (
        attempt.phase !== "ready" &&
        now - attempt.phaseStarted >=
          (attempt.phase === "connecting" ? this.#profile.connectMs : this.#profile.helloMs)
      ) {
        return this.#retire(
          attempt,
          new Error(
            `session ${attempt.phase === "connecting" ? "connection" : "HELLO"} deadline exceeded`,
          ),
          `${attempt.phase}-unavailable`,
        );
      }
      const pending = attempt.probe;
      if (
        attempt.controls.some((item) => now - item.created >= this.#profile.controlSubmitMs) ||
        (pending?.submitted === null && now - pending.created >= this.#profile.controlSubmitMs)
      ) {
        return this.#retire(
          attempt,
          new Error("connection control submission stalled"),
          "local-control-stall",
        );
      }
      if (pending?.submitted !== null && pending?.submitted !== undefined) {
        const elapsed = now - pending.submitted;
        if (elapsed >= this.#profile.responseMs)
          return this.#retire(
            attempt,
            new Error("session PONG response unavailable"),
  ...
  #listenSocket(attempt) {
    ...
    const close = () => {
      ...
      if (this.#socket === socket)
        this.#retire(this.#attempt, new Error("session socket closed"), "socket-close");
      else this.#admit();
    };
    socket.addEventListener("close", close);
  }
  ...
  #resetOutputDecoder(record) {
    ...
    record.outputDecoder = new Decompress((chunk) => {
      ...
      while (
        ...
        record.outputPreambleOffset < OUTPUT_STREAM_PREAMBLE.byteLength
      ) {
        if (chunk[offset] !== OUTPUT_STREAM_PREAMBLE[record.outputPreambleOffset])
          throw new Error("invalid output stream preamble");
        offset++;
        record.outputPreambleOffset++;
      }
      const output = chunk.subarray(offset);
      if (record.outputDecodeOffset + output.byteLength > target.byteLength)
        throw new Error("output decoder overflow");
  ...
  #decodeOutput(record, body, rawLength) {
    ...
    if (!this.#eventScratch || this.#eventScratch.byteLength < rawLength)
      this.#eventScratch = new Uint8Array(rawLength);
    ...
    try {
      ...
      if (
        record.outputPreambleOffset !== OUTPUT_STREAM_PREAMBLE.byteLength ||
        record.outputDecodeOffset !== rawLength
      )
        throw new Error("output decoder length mismatch");
      return target;
    } finally {
      record.outputDecodeTarget = null;
    }
  ...
  #current(attempt) {
    return (
      !!attempt && attempt === this.#attempt && !attempt.retired && !this.#disposed && !this.#fatal
    );
  }
  ...
  #retire(attempt, error, outcome) {
    ...
    this.#state.lastOutcome = {
      ...
      at: now,
      bufferedAmount: attempt.socket?.bufferedAmount ?? 0,
      queuedApplicationBytes: attempt.queueBytes,
      queuedSendBytes: attempt.controlBytes + attempt.sendBytes,
      probeSubmittedAt: attempt.probe?.submitted ?? null,
      fatal: !!this.#fatal,
    };
    ...
    if (!this.#disposed && !this.#fatal) {
      const base = Math.min(
        this.#profile.retryCeilingMs,
        this.#profile.retryFloorMs * 2 ** Math.min(this.#failures++, 16),
      );
    ...
    for (const record of this.#records.values())
      if (record.attempt === null) this.#attachmentEmitter.emit(record, "detached");
  }
  ...
  #scheduleAdmission() {
    if (
      ...
      return;
    if (this.#slots.size >= 2 && this.#clock.now() >= this.#retryAt)
      return this.#setStatus("resource-wait", false);
  ...
  #removeRecord(record, error) {
    ...
    if (
      ...
      attempt.canceledAttaches.set(record.attachmentId.toString(), {
        ...
        attachRequestId: record.attachRequestId,
        epoch: 0n,
      });
    }
    this.#cancelRecordSends(record);
  ...
  #cancelRecordSends(record) {
    ...
    attempt.sends = attempt.sends.filter(
      (item) => item.attachmentId !== record.attachmentId || item.type === FrameType.DETACH,
    );
  ...
  #requireRecord(record, frame) {
    if (
      ...
      !record.active ||
      record.attempt !== this.#attempt ||
      this.#records.get(frame.attachmentId.toString()) !== record ||
      !equalBytes(frame.sessionId, record.sessionId) ||
      (record.epoch === 0n
        ? frame.type !== FrameType.ATTACH_BEGIN
        : frame.attachmentEpoch !== record.epoch)
    )
      throw new Error("stale attachment frame");
  }

  #nextRequestId() {
    this.#requestSequence =
      this.#requestSequence === (1n << 64n) - 1n ? 1n : this.#requestSequence + 1n;
