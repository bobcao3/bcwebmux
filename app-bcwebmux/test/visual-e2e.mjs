...
const source = (await readFile(path.join(testDir, "snapshot-fixture.zig"), "utf8")).trimEnd().split("\n");
const devices = [
...
function numberedSource(lines, start = 0) {
  return lines.map((line, i) => `\x1b[90m${String(start + i + 1).padStart(3)} │ \x1b[${line.trim().startsWith("//") ? "32" : "37"}m${line}\x1b[0m`).join("\n");
}
...
async function run(backend, device) {
  ...
  try {
    ...
    chromium.on("error", error => { processError = error; });
    chromium.stderr.on("data", chunk => { log += chunk; });
    ...
    }, 15000, "Chromium did not expose visual page");
    ...
    assert.deepEqual(await evaluate("({width: innerWidth, height: innerHeight, dpr: devicePixelRatio})"), { width: device.width, height: device.height, dpr: device.deviceScaleFactor });
    ...
    const output = async (text, reset = false) => {
      ...
      await until(`document.querySelector('#terminal-identity-primary').textContent === ${JSON.stringify(title)}`, "PTY fixture did not finish");
    };
    const capture = async (label, goldenLabel = label) => {
      if (!device.mobile) await until(`(() => {
      ...
      await evaluate("(async () => { await window.bcwebmux.readPixels(); await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))); })()");
      let previous;
      const png = await waitFor(async () => {
        const { data } = await page.call("Page.captureScreenshot", { format: "png", fromSurface: true, captureBeyondViewport: false });
        if (data === previous) return Buffer.from(data, "base64");
        previous = data;
        return null;
      }, 8000, `${label}: compositor did not settle`);
      ...
      assert.equal(geometry.width, Math.round(geometry.cssWidth * device.deviceScaleFactor), 'canvas is not rendered at native DPR');
      assert.equal(geometry.height, Math.round(geometry.cssHeight * device.deviceScaleFactor), 'canvas is not rendered at native DPR');
      await writeFile(path.join(outputDir, `${name}.json`), JSON.stringify({ device, backend, state: diagnostics }, null, 2));
      try {
        results[label] = await compareScreenshot({ png, name, goldenName: `${prefix}-${goldenLabel}`, goldenDir, outputDir,
          width: device.width * device.deviceScaleFactor, height: device.height * device.deviceScaleFactor,
    ...
    await until("window.bcwebmux.state.textRenderer === 'kb-stb'", "default text renderer was not restored");
    await output(`\x1b[1;36mSCROLLBACK START · snapshot-fixture.zig\x1b[0m\n${numberedSource(source)}\n\x1b[1;33mSECOND PASS · snapshot-fixture.zig\x1b[0m\n${numberedSource(source)}\n\x1b[1;35mSCROLLBACK END · UTF-8: λ café\x1b[0m\n`, true);
    await until("window.bcwebmux.state.scrollTotal > window.bcwebmux.state.scrollLength + 20 && window.bcwebmux.state.scrollOffset + window.bcwebmux.state.scrollLength === window.bcwebmux.state.scrollTotal", "source did not fill scrollback");
    ...
    await until("window.bcwebmux.state.scrollOffset === 0 && window.bcwebmux.state.viewportMode === 'top'", "Home did not reach oldest history");
    ...
    await until("window.bcwebmux.state.scrollOffset > 0 && window.bcwebmux.state.scrollOffset + window.bcwebmux.state.scrollLength < window.bcwebmux.state.scrollTotal", "PageDown did not reach middle history");
    ...
    if (page) {
      const capture = await page.call("Page.captureScreenshot", { format: "png" }).catch(() => null);
      if (capture) await writeFile(path.join(outputDir, `${prefix}-failure.png`), Buffer.from(capture.data, "base64"));
    }
