...
try {
  ...
  const terminated = await request("POST", `/api/sessions/${session.id}/terminate`, {
    origin,
    "idempotency-key": "https-terminate",
  });
  ...
  const result = await Promise.race([
    exited,
    delay(7000, undefined, { ref: false }).then(() => {
      throw new Error(`shutdown timed out with an open WSS connection\n${logs}`);
    }),
  ]);
  assert.equal(result.code, 0, `unclean shutdown ${JSON.stringify(result)}\n${logs}`);
  console.log(
    "Go HTTPS integration passed: TLS, HTTP/2, embedded assets, origin checks, cgo session lifecycle, WSS, shutdown",
  );
