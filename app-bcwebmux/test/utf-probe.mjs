...
const webRoot = process.argv[2]
  ...
  : new URL("../zig-out/web/", import.meta.url);
...
function capturePacket(submissionPtr) {
  ...
  if (
    submission.getUint32(0, true) !== 0x5355424d ||
  ...
  for (let i = 0; i < bitmapRequestCount; i++) {
    ...
    for (let slotIndex = 0; slotIndex < slotCount; slotIndex++) {
      ...
      bitmaps.push({
        ...
        height: 16,
        mask,
        nonZero: mask.reduce((a, b) => a + (b > 0 ? 1 : 0), 0),
      });
    }
  }
...
function renderFrame() {
  ...
  try {
    ...
    e.term_frame_finish(token, accepted ? 1 : 0);
  }
}
const bitmaps = [];
const frames = [];
...
function utf8For(cp) {
  ...
  if (cp <= 0x7ff) return String.fromCharCode(0xc0 | (cp >> 6), 0x80 | (cp & 0x3f));
  if (cp <= 0xffff)
...
function report(label, codepoints = []) {
  ...
  console.log(`\n=== ${label} ===`);
  console.log(
    `frame cols=${frames.at(-1)?.cols} rows=${frames.at(-1)?.rows} cells=${cellList.length}`,
  );
