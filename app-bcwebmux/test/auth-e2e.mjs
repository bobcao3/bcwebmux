...
async function signInWithCode(secret) {
  await until(
    ...
    "login page did not finish initializing",
  );
  await evaluate(
...
async function addKeyFromPanel(label, code = null) {
  ...
  if (code !== null) {
    await until(
      `!document.querySelector("#auth-stepup-row").hidden`,
      "the panel did not ask for a code",
    );
    await evaluate(
      `document.querySelector("#auth-stepup-code").value = ${JSON.stringify(code)}; document.querySelector("#auth-stepup-form").requestSubmit()`,
      true,
    );
  }
  await until(
    `document.querySelector("#auth-credential-list").textContent.includes(${JSON.stringify(label)})`,
    `enrolling "${label}" did not complete`,
    30000,
  );
  await until(
    `!document.querySelector("#auth-add-key").disabled`,
    "key management did not become idle again",
  );
}
...
try {
  ...
  assert.equal(
    ...
    0,
    "no security key can be enrolled from the host",
  );
  ...
  await until(
    `document.querySelector("#auth-subtitle").textContent === "Sign in to continue" && !document.querySelector("#auth-code-form label") && !/relying party/i.test(document.querySelector("#auth-card").textContent)`,
    "the login page must not name the relying party or repeat the code label",
  );
  await capture("login");
  ...
  await until(
    ...
    "settings panel did not list the authenticator app",
  );
  assert.equal(
    await evaluate(`document.querySelectorAll("#auth-credential-list .auth-credential").length`),
    1,
    "the panel must show the authenticator app and no keys",
  );
  ...
  await evaluate(
    `document.querySelector("#auth-stepup-code").value = "000000"; document.querySelector("#auth-stepup-form").requestSubmit()`,
    true,
  );
  await until(
    `/not valid/.test(document.querySelector("#auth-status").textContent)`,
    "a wrong code was not reported",
    10000,
  );
  assert.equal(await evaluate(`location.pathname`), "/", "a wrong code must not end the session");
  await until(
    `!document.querySelector("#auth-add-key").disabled`,
    "the panel did not become idle again after a wrong code",
  );
  ...
  assert.equal(
    await evaluate(`document.querySelector("#auth-key-name").value`),
    "",
    "the label field must be cleared after enrolling",
  );
  ...
  await until(
    `document.querySelectorAll("#auth-credential-list .auth-credential").length === 3`,
    "adding a second key did not update the list",
    30000,
  );
  assert.match(
    await evaluate(`document.querySelector("#auth-summary").textContent`),
    /the authenticator app and 2 security keys/,
    "the summary must count both keys",
  );
  ...
  assert.deepEqual(withTwo.credentials.map((credential) => credential.name).sort(), [
    "macbook · touch id",
    "pixel · fingerprint",
  ]);
  assert.ok(
    withTwo.credentials.every((credential) => credential.rpId === "localhost"),
    "both keys must be scoped to the origin host",
  );
  ...
  await until(
    `globalThis.bcwebmuxAuth?.state.ready === true`,
    "login page did not finish initializing after signing out",
  );
  await until(
    `!document.querySelector("#auth-action").hidden`,
    "the key button must be offered once a key is enrolled",
  );
  ...
  await until(
    `typeof window.bcwebmux === "object"`,
    "application modules did not load after the key sign-in",
    30000,
  );
  ...
  await until(
    `document.querySelectorAll("#auth-credential-list .auth-credential").length === 3`,
    "the panel did not list both keys",
  );
  ...
  await waitFor(
    () => Math.floor(Date.now() / 30000) > firstStepUpCounter,
    65000,
    "no unused TOTP step became available",
  );
  await addKeyFromPanel("phone · passkey", totp(secret, Math.floor(Date.now() / 1000)));
  assert.equal(
    JSON.parse(await readFile(authFile, "utf8")).credentials.length,
    3,
    "a code must allow enrollment even with existing keys",
  );
  const clickRemoval = (name) =>
    evaluate(
      `(() => {
  ...
  await until(
    `document.querySelectorAll("#auth-credential-list .auth-credential").length === 3`,
    "new key was not removed",
  );
  await clickRemoval("pixel · fingerprint");
  await until(
    `/Click CONFIRM/.test(document.querySelector("#auth-status").textContent)`,
    "removal was not armed",
  );
  await clickRemoval("pixel · fingerprint");
  await until(
    `location.pathname === "/login" || document.querySelectorAll("#auth-credential-list .auth-credential").length === 2`,
    "removing a key did not update the list or end its session",
    30000,
  );
  const afterRemoval = JSON.parse(await readFile(authFile, "utf8"));
  assert.deepEqual(
    afterRemoval.credentials.map((credential) => credential.name),
    ["macbook · touch id"],
    "the state file must drop only the removed key",
  );
  assert.ok(afterRemoval.sessionSecret, "the authenticator app keeps the state file signed");

  console.log(
    "auth-e2e: CLI authenticator enrollment, code sign-in, per-device key enrollment from the panel, key sign-in, and removal verified",
  );
} catch (error) {
