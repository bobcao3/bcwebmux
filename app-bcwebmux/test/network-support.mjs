  ...
  wait(predicate, timeout = 10000, after = 0) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => finish(new Error(`event deadline exceeded (${timeout}ms)`)),
        timeout,
      );
...
export async function waitForRestoredController(journal, { after, deadlineAt, ready }) {
  ...
  for (;;) {
    ...
    const event = await journal.wait(
      (event) => ["state", "welcome", "live-barrier"].includes(event.type),
      remaining,
      cursor,
    );
...
export function spawnListening(command, args, pattern, timeout = 20000) {
  ...
  const ready = new Promise((resolve, reject) => {
    ...
    const exit = (code, signal) =>
      finish(new Error(`${command} exited before readiness: ${code}/${signal}\n${log}`));
    const failure = (error) => finish(error);
    const timer = setTimeout(
      () => finish(new Error(`${command} readiness timeout\n${log}`)),
      timeout,
    );
    const data = (bytes) => {
