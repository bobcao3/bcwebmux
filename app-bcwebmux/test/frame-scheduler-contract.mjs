...
let consumed = 0, presented = 0, blinking = false;
const host = {
  _renderer: { presentationOpportunityMs: null },
  _renderFrame() { consumed++; return 1; },
  _presenter: {
    present() { presented++; },
    nextAnimationDeadline(time) { return blinking ? (Math.floor(time / 500) + 1) * 500 : null; },
  },
};
...
scheduler.requestPresentation(); scheduler.requestPresentation(); frame();
assert.equal(consumed, 0); assert.equal(presented, 1);
assert.equal(host._renderer.presentationOpportunityMs, 0, "scheduler owns presentation-opportunity sampling");
scheduler.schedule(); scheduler.schedule(); scheduler.requestPresentation(); frame();
assert.equal(consumed, 1); assert.equal(presented, 2);
...
scheduler.schedule(true); scheduler.schedule(true);
assert.equal(consumed, 1); frame(); assert.equal(consumed, 2);
scheduler.suspend(); scheduler.resume(); frame();
...
blinking = true; scheduler.requestPresentation(); frame();
assert.equal(timers.size, 1);
const [key, timer] = timers.entries().next().value;
assert.equal(timer.at, 500);
now = 500; timers.delete(key); timer.fn();
const before = consumed; frame();
assert.equal(consumed, before, "blink does not consume core");
assert.equal(timers.values().next().value.at, 1000);
document.hidden = true; scheduler.suspend();
assert.equal(frames.size + timers.size, 0);
scheduler.schedule(); scheduler.requestPresentation();
assert.equal(frames.size + timers.size, 0);
document.hidden = false; scheduler.resume(); frame();
...
scheduler.dispose();
assert.equal(frames.size + timers.size, 0);
scheduler.schedule(true); scheduler.resume(); scheduler.requestPresentation();
assert.equal(frames.size + timers.size, 0);
