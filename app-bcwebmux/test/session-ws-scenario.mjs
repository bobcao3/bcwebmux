...
export async function runSessionWebSocketScenario(serverPath, deps) {
  ...
  try {
    ...
    await waitFor(
      async () => (await workerPids(server.pid)).length > 0,
      3000,
      "session worker did not start",
    );
    ...
    let metadata = await waitSession(
      base,
      session.id,
      (value) =>
        value.attachments === 1 &&
        value.controller?.attachmentId === attachmentText(aAttachment.id),
      "A controller metadata",
    );
    ...
    const retainedCursor = {
      eventSeq: aAttachment.eventSeq,
      outputOffset: aAttachment.outputOffset,
    };
    await a.close();
    await waitSession(
      base,
      ...
      (value) =>
        ...
        value.attachments === 0,
      "session detached after A close",
    );

    const b = await new RawClient(wsUrl, base, "B").connect();
    clients.push(b);
    const bAttachment = await b.attach(
      session,
      session.generation,
      retainedCursor.eventSeq,
      retainedCursor.outputOffset,
      0x202n,
    );
    ...
    await a2.connect();
    clients.push(a2);
    const a2Attachment = await a2.attach(
      session,
      session.generation,
      retainedCursor.eventSeq,
      retainedCursor.outputOffset,
    ...
    assert.ok(
      sameBytes(a2Replay, detachedTail),
      `replayed detached tail differs or duplicated: A-replay length=${a2Replay.byteLength} value=${JSON.stringify(decoder.decode(a2Replay))}; B-tail length=${detachedTail.byteLength} value=${JSON.stringify(decoder.decode(detachedTail))}`,
    );
    ...
    const observerAttachment = await observer.attach(
      session,
      session.generation,
      a2Attachment.eventSeq,
      a2Attachment.outputOffset,
      0x404n,
    );
    ...
    metadata = await waitSession(
      base,
      session.id,
      (value) =>
        value.attachments === 3 &&
        value.controller?.attachmentId === attachmentText(a2Attachment.id),
      "latest controller metadata",
    );
    ...
    assert.equal(
      await a2.input(a2Attachment, "printf 'CURRENT-OUTPUT-9\\n'; printf 'CURRENT-'\"END\\n\"\n"),
      0,
    );
    ...
    assert.ok(
      sameBytes(
        a2.outputSince(a2Attachment, aOutputStart),
        observer.outputSince(observerAttachment, observerOutputStart),
      ),
      "observer output diverged",
    );
    ...
    assert.equal(
      await a2.input(
        a2Attachment,
        `for i in 1 2 3 4 5 6; do cat ${shellQuote(sessionSocketPath)}; done; printf 'PTY-COMP'\"RESSION-END\\n\"\n`,
      ),
      0,
    );
    ...
    const compressionCompressedDelta =
      a2Attachment.outputCompressedBytes - compressionCompressedStart;
    ...
    assert.ok(
      compressionRawDelta > 4096 && compressionWireDelta > 0 && compressionRatio >= 8,
      `PTY compression regression: raw=${compressionRawDelta}, compressed=${compressionCompressedDelta}, events=${compressionEventDelta}, bodyRatio=${compressionBodyRatio}, fullWireRatio=${compressionRatio}`,
    );
    ...
    metadata = await waitSession(
      base,
      session.id,
      (value) => value.geometry.cols === 100 && value.geometry.rows === 33,
      "resize metadata before ordered event",
    );
    const resizeEvent = await a2.waitEvent(
      a2Attachment,
      (event) => event.kind === 1,
      "ordered resize event",
    );
    assert.ok(resizeEvent.seq > resizeBefore);
    assert.equal(resizeEvent.seq, a2Attachment.eventSeq);
    assert.deepEqual(resizeEvent.geometry, {
      cols: 100,
      rows: 33,
      cellWidthPx: 8,
      cellHeightPx: 16,
    ...
    assert.equal(
      await a2.input(a2Attachment, "printf 'SIZE='; stty size; printf '\\nSIZE-'\"END\\n\"\n"),
      0,
    );
    ...
    metadata = await waitSession(
      ...
      session.id,
      (value) =>
        value.geometry.cols === 100 &&
        value.geometry.rows === 33 &&
        value.controller?.attachmentId === attachmentText(a2Attachment.id),
      "resize metadata",
    );
    ...
    const lowCreditError = await lowCredit.attachExpectError(
      session,
      session.generation,
      0n,
      0n,
    ...
    const freshReplayAttachment = await freshReplay.attach(
      session,
      session.generation,
      0n,
      0n,
    ...
    assert.deepEqual(freshReplayAttachment.replayGeometry, {
      cols: 80,
      rows: 24,
      cellWidthPx: 8,
      cellHeightPx: 16,
    });
    assert.deepEqual(freshReplayAttachment.geometry, {
      cols: 100,
      rows: 33,
      cellWidthPx: 8,
      cellHeightPx: 16,
    ...
    clients.push(slow);
    const slowAttachment = await slow.attach(
      session,
      session.generation,
      a2Attachment.eventSeq,
      a2Attachment.outputOffset,
    ...
    assert.ok(
      sameBytes(
        slow.outputSince(slowAttachment, slowFlowStart),
        a2.outputSince(a2Attachment, a2FlowStart),
      ),
    );
    ...
    const terminateResponse = await fetch(`${base}/api/sessions/${session.id}/terminate`, {
      method: "POST",
      headers: mutationHeaders(base, "ws-protocol-terminate"),
    });
    ...
    metadata = await waitSession(
      ...
      (value) =>
        value.state === "exited" &&
        value.checkpointBytes > 0 &&
        value.checkpointEventSeq === value.eventSeq,
      "session exit checkpoint",
    );
    ...
    assert.equal(
      finalAttachment.mode,
      1,
      "fresh retained-generation attach should restore checkpoint",
    );
    ...
    await Promise.all([
      b.detach(bAttachment),
      a2.detach(a2Attachment),
      observer.detach(observerAttachment),
      freshReplay.detach(freshReplayAttachment),
      slow.detach(slowAttachment),
      finalClient.detach(finalAttachment),
    ]);
    await waitSession(
      base,
      session.id,
      (value) => value.attachments === 0 && value.state === "exited",
      "all websocket attachments detached",
    );
    const deleted = await fetch(`${base}/api/sessions/${session.id}`, {
      method: "DELETE",
      headers: mutationHeaders(base, "ws-protocol-delete"),
    });
    ...
    if (session && server.exitCode === null) {
      await fetch(`${base}/api/sessions/${session.id}/terminate`, {
        method: "POST",
        headers: mutationHeaders(base, "ws-protocol-cleanup"),
      }).catch(() => {});
      await waitSession(
        base,
        session.id,
    ...
    if (workerPid)
      await waitFor(
        async () => !(await processState(workerPid)),
        1500,
        () => `worker ${workerPid} leaked after shutdown\n${logs}`,
      );
  }
}
