...
async function event(target, name, timeout = 3000) {
  ...
  try { return await once(target, name, { signal: controller.signal }); }
...
async function destroy(socket) {
  ...
  const closed = event(socket, "close"); socket.destroy(); await closed;
}
async function echoTcp(prefix) {
  ...
  server.listen(0, "127.0.0.1"); await event(server, "listening");
  return { port: server.address().port, async close() { await Promise.all([...sockets].map(destroy)); await new Promise(resolve => server.close(resolve)); } };
}

test("TCP latency, reset, changed target/source ports, silent stale mapping and bounded cleanup", { timeout: 10000 }, async () => {
  const a = await echoTcp("A"), b = await echoTcp("B");
  ...
  try {
    ...
    const started = performance.now(); socket.write("first");
    assert.equal((await response)[0].toString(), "Afirst");
    assert.ok(performance.now() - started >= 35, "both 20ms directions impaired");
    const oldSourcePort = relay.history.find(entry => entry.type === "tcp-connected").sourcePort;
    // RST is expected: await close directly rather than events.once's error rejection.
    const closed = new Promise(resolve => socket.once("close", resolve));
    const fault = relay.fault("reset"); await closed;
    const restored = relay.restore({ upstreamPort: b.port });
    assert.ok(restored.at >= fault.at);
    socket = await tcpClient(relay.port);
    response = event(socket, "data"); socket.write("second");
    assert.equal((await response)[0].toString(), "Bsecond");
    assert.notEqual(relay.history.filter(entry => entry.type === "tcp-connected").at(-1).sourcePort, oldSourcePort);
    relay.fault("blackhole"); relay.restore();
    ...
    socket = await tcpClient(relay.port);
    response = event(socket, "data"); socket.write("new-path");
    ...
    const queued = new Promise(resolve => {
      ...
      relay.schedule = (...args) => { original(...args); resolve(); };
    });
    socket.write("cancel-me"); await queued;
    assert.ok(relay.bufferedBytes > 0);
    await relay.close();
    assert.equal(relay.bufferedBytes, 0); assert.equal(relay.timers.size, 0); assert.equal(relay.pendingCloses.size, 0);
  } finally { await relay.close(); if (socket) await destroy(socket); await a.close(); await b.close(); }
});

test("TCP lowering latency preserves admitted byte order in both directions", { timeout: 10000 }, async () => {
  ...
  relay.schedule = (...args) => { schedule(...args); relay.emit("admitted", args); };
  try {
    socket = await tcpClient(relay.port);
  ...
  } finally { await relay.close(); if (socket) await destroy(socket); await echo.close(); }
});

for (const action of ["reset", "blackhole", "close", "overflow"]) {
  test(`TCP ready followers remain accounted and are cancelled on ${action}`, { timeout: 10000 }, async () => {
    const echo = await echoTcp("");
    const relay = await new FaultRelay({ upstreamPort: echo.port, latencyMs: 5000, maxBufferedBytes: 11 }).listen();
    let socket;
    const schedule = relay.schedule.bind(relay);
    relay.schedule = (...args) => { schedule(...args); relay.emit("admitted", args); };
    const drain = relay.drainBuffers.bind(relay);
    relay.drainBuffers = direction => { drain(direction); relay.emit("ready", direction); };
    try {
      socket = await tcpClient(relay.port);
      let admitted = event(relay, "admitted");
      socket.write("earlier"); await admitted;
      relay.setLatency(0);
      const ready = event(relay, "ready");
      socket.write("late");
      const [direction] = await ready;
      assert.equal(direction.items.size, 2);
      assert.deepEqual([...direction.items].map(item => item.ready), [false, true]);
      assert.equal(relay.bufferedBytes, 11);
      assert.equal(direction.queued, 11);
      assert.equal(relay.timers.size, 2, "expired follower remains in cancellation/accounting registry");
      if (action === "close") await relay.close();
      else if (action === "overflow") {
        admitted = event(relay, "admitted");
        socket.write("!"); await admitted;
      ...
      await relay.close(); if (socket) await destroy(socket); await echo.close();
      assert.equal(relay.pendingCloses.size, 0);
    }
  });
}

test("UDP symmetric latency and actual upstream source-port rebinding on same downstream socket", { timeout: 10000 }, async () => {
  const echo = dgram.createSocket("udp4");
  const ports = [];
  echo.on("message", (bytes, peer) => { ports.push(peer.port); echo.send(bytes, peer.port, peer.address); });
  echo.bind(0, "127.0.0.1"); await event(echo, "listening");
  const relay = await new FaultRelay({ protocol: "udp", upstreamPort: echo.address().port, latencyMs: 20 }).listen();
  const client = dgram.createSocket("udp4");
  client.bind(0, "127.0.0.1"); await event(client, "listening");
  try {
    let received = event(client, "message"); const started = performance.now();
    ...
    received = event(client, "message"); client.send(Buffer.from("after"), relay.port, "127.0.0.1");
    assert.equal((await received)[0].toString(), "after");
    assert.notEqual(ports[0], ports[1]); assert.equal(client.address().port, downstreamPort);
    relay.fault("blackhole"); assert.equal(relay.timers.size, 0); relay.restore();
    received = event(client, "message"); client.send(Buffer.from("restored"), relay.port, "127.0.0.1");
    assert.equal((await received)[0].toString(), "restored");
  } finally {
    await relay.close();
    const clientClosed = event(client, "close"), echoClosed = event(echo, "close");
    client.close(); echo.close(); await Promise.all([clientClosed, echoClosed]);
    assert.equal(relay.bufferedBytes, 0); assert.equal(relay.pendingCloses.size, 0);
  }
});
...
test("relay rejects unbounded configurations and overflow cancels queued buffers", () => {
  ...
  assert.equal(flow.closed, true); assert.equal(relay.bufferedBytes, 0); assert.equal(relay.timers.size, 0);
});

test("CDP subscriptions and journal waits are event driven and dispose on disconnect", { timeout: 10000 }, async () => {
  ...
  const closed = assert.rejects(journal.wait(() => false), /closed/);
  await journal.close();
  await closed;
