...
assert.ok(
  ...
  "DEVICE_SCALE_FACTOR must be a positive number",
);
...
try {
  ...
  const target = await waitFor(
    async () => {
      ...
      return targets.find(
        (item) =>
          item.type === "page" && item.url.includes(`terminal-core/index.html?backend=${backend}`),
      );
    },
    15000,
    () => `Chromium failed to expose test page\n${chromiumLog}`,
  );
  ...
  const response = await pageCdp.call("Runtime.evaluate", {
    expression:
      '(async () => { const end = performance.now() + 10000; while (!window.terminalCoreTest && performance.now() < end) await new Promise((resolve) => setTimeout(resolve, 50)); if (!window.terminalCoreTest) throw new Error("terminal core integration did not initialize"); return await window.terminalCoreTest; })()',
    awaitPromise: true,
    returnByValue: true,
  });
  if (response.exceptionDetails) {
    const screenshot = await pageCdp
    ...
    throw new Error(
      `${response.exceptionDetails.exception?.description || "terminal core integration failed"}\n${serverLog}`,
    );
  }
  ...
  assert.equal(
    result.atlas.textureBytes,
    result.atlas.capacity * result.physicalCellWidth * result.physicalCellHeight,
  );
  assert.equal(result.coreCount, 2);
  const coreA = result.partitions.coreA;
  const coreB = result.partitions.coreB;
  assert.ok(coreA.base + coreA.capacity <= coreB.base || coreB.base + coreB.capacity <= coreA.base);
  ...
  assert.ok(
    result.colorA[0] > result.colorA[1] * 2 && result.colorA[0] > result.colorA[2] * 2,
    JSON.stringify(result),
  );
  assert.ok(
    result.colorB[1] > result.colorB[0] * 2 && result.colorB[2] > result.colorB[0] * 2,
    JSON.stringify(result.colorB),
  );
  ...
  console.log(
    JSON.stringify({ terminalCore: "ok", backend, deviceScaleFactor, canvasFontLifecycle: true }),
  );
} finally {
  pageCdp?.close();
  browserCdp?.close();
