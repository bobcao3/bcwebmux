...
test("pixel comparison tolerates small channel noise but detects local missing content", () => {
  ...
  assert.equal(
    pixelDifference(Buffer.alloc(expected.length, 12), expected, 1024, 1024).changedRatio,
    0,
  );
  const actual = Buffer.from(expected);
  for (let y = 0; y < 12; y++)
    for (let x = 0; x < 12; x++) actual.fill(255, (y * 1024 + x) * 3, (y * 1024 + x) * 3 + 3);
  const result = pixelDifference(actual, expected, 1024, 1024);
  assert.ok(result.changedRatio < 0.002);
  assert.ok(
    result.maxTileRatio > 0.02,
    "local regression must not be diluted by the full viewport",
  );
});
...
test("screenshot artifacts, explicit updates, missing baselines and viewport dimensions", async () => {
  ...
  const options = {
    ...
    outputDir: path.join(dir, "actual"),
    width: 64,
    height: 64,
  };
  const png = await sharp({ create: { width: 64, height: 64, channels: 3, background: "black" } })
    .png()
    .toBuffer();
  try {
    ...
    assert.equal(
      (await compareScreenshot({ ...options, name: "returned", goldenName: "test", png }))
        .changedRatio,
      0,
    );
