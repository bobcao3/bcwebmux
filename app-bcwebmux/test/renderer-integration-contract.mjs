...
} from "../../wgpuTerminal/src/browser/render/RenderBackend.js";
...
for (const path of [
  "browser/ViewportController.js",
  "browser/input/PointerController.js",
  "browser/input/InputController.js",
]) {
  assert.doesNotMatch(
    await readFile(new URL(path, root), "utf8"),
    /getCore|TerminalCore|\.term_|\._wasm/,
  );
}
  ...
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) await inspect(url);
      ...
      if (/\/(webgpu|webgl)\//.test(url.pathname))
        assert.doesNotMatch(
          source,
          /requestAnimationFrame|setInterval|_scheduler|submissionMetadata/,
        );
    }
  }
}
await inspect(root);
assert.match(
  await readFile(new URL("browser/render/CanvasAlphaMask.js", root), "utf8"),
  /fillText/,
);
assert.doesNotMatch(
  await readFile(new URL("browser/render/CanvasAlphaMask.js", root), "utf8"),
...
const lostGl = {
  ...
  gl: new Proxy(
    ...
    {
      get() {
        assert.fail("must not delete invalid pre-restoration GL handles");
      },
    },
  ),
};
...
FramePresenter.prototype.registerTerminal = function (core, cells) {
  this.backend.glyphPartitions.set(core, {
    baseSlot: this.backend.glyphPartitions.size * 64,
    slotCapacity: cells,
    generation: 2,
  });
};
...
function backend(kind) {
  return {
    ...
    initialized: false,
    atlasColumns: 16,
    glyphPartitions: new Map(),
    coreSwitches: 0,
    setPhysicalCellMetrics(...args) {
      this.metrics = args;
    },
    setGrainStrength(value) {
      this.grain = value;
    },
    async initialize(capacity) {
...
try {
  for (const kind of ["webgpu", "webgl2"]) {
    ...
    terminal._renderMetrics = Object.freeze({
      generation: 1,
      cols: 8,
      rows: 3,
      cellWidth: 8,
    ...
    const cores = [0, 1].map((id) => ({
      ...
      reset() {
        assert.fail("must not reset terminal");
      },
      write() {
        assert.fail("must not replay bytes");
      },
    }));
    terminal._cores = new Set(cores);
    terminal._core = cores[0];
  ...
  try {
    ...
    disposedTerminal._renderer = await acquireRenderBackend(
      disposedTerminal,
      {},
      {},
      "kb-stb",
...
try {
  ...
  await assert.rejects(
    GpuTerminal.create({}, {}, "kb-stb", undefined, "high-performance"),
    /adapter unavailable/,
  );
...
await assert.rejects(
  WebGlTerminal.create(unavailableCanvas, {}, "kb-stb", undefined, "low-power"),
  /context unavailable/,
);
...
console.log(
  "renderer integration contract passed (semantic controllers, recovery, ownership, power hint)",
);
