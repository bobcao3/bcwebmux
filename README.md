...
Drag to select terminal text when application mouse reporting is inactive. On touch screens, a long press uses native selection handles and copies through libghostty selection semantics. When a TUI captures mouse input, hold Shift before pressing to force local selection. Copy with Ctrl+Shift+C or Meta+C; plain Ctrl+C is sent to the terminal application.
