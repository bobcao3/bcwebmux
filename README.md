...
OSC 8 links open only after a modal displays the complete destination, and only absolute HTTP(S) links are allowed. OSC 9/777 can produce native notifications only while the page is open after bcwebmux presents a one-time default opt-in prompt; permission can also be managed from Settings. This uses the browser Notifications API, not background Web Push. OSC 52 standard text clipboard writes and clears use the browser Clipboard API; primary/selection destinations and clipboard read queries are intentionally unsupported, with reads suppressed to prevent terminal programs silently exfiltrating clipboard contents.
